...
local Custom = {} do
    ...
    Custom.AccentColor = Color3.fromRGB(25, 25, 112)
...
function Speed_Library:SetNotification(Config)
	...
	local lines = math.ceil(TextLabel2.TextBounds.X / TextLabel2.AbsoluteSize.X)
	TextLabel2.Size = UDim2.new(1, -20, 0, 13 + (13 * lines))
...
function Speed_Library:CreateWindow(Config)
  ...
  local SpeedHubXGui = Custom:Create("ScreenGui", {
    ...
    local DropShadowHolder = Custom:Create("Frame", {
        ...
        Position = UDim2.new(0, math.floor(SpeedHubXGui.AbsoluteSize.X / 2 - 455 / 2), 0, math.floor(SpeedHubXGui.AbsoluteSize.Y / 2 - 350 / 2))
    }, SpeedHubXGui)
...
-- Control Buttons Container (Hapus semua kode GreenButton & Mac logic)
    local Controls = Custom:Create("Frame", {
        BackgroundTransparency = 1,
        AnchorPoint = Vector2.new(1, 0.5),
    ...
    local UIListLayoutControls = Custom:Create("UIListLayout", {
	...
	local SearchResultPage = Custom:Create("ScrollingFrame", {
		Size = UDim2.new(1, 0, 1, 0),
		BackgroundTransparency = 1,
		Visible = false,
		Name = "SearchResultPage",
		Parent = LayersReal -- Sejajar sama Tab-Tab lain
	})

	Custom:Create("UIListLayout", { Padding = UDim.new(0, 5) }, SearchResultPage)
	...
	local SearchFrame = Custom:Create("Frame", {
		...
		Position = UDim2.new(0, 5, 0, 5), -- Kasih margin dikit
		Name = "SearchFrame",
		Parent = ScrollTab -- Masukin ke scrollbar biar ikut gerak atau LayersTab biar diem
	})

	local SearchInput = Custom:Create("TextBox", {
	...
	-- Tambahkan UI Corner biar estetik mac-style
...
-- Background gelap transparan buat nutupin seluruh UI pas konfirmasi muncul
local ConfirmOverlay = Custom:Create("Frame", {
    Size = UDim2.new(1, 0, 1, 0),
    BackgroundColor3 = Color3.fromRGB(0, 0, 0),
    BackgroundTransparency = 1, -- Defaultnya sembunyi
    Visible = false,
    ZIndex = 10,
    Name = "ConfirmOverlay"
}, Main) -- Masukin ke Main frame kamu

-- Kotak Dialog Tengah
local ConfirmFrame = Custom:Create("Frame", {
    AnchorPoint = Vector2.new(0.5, 0.5),
...
Custom:Create("UICorner", {CornerRadius = UDim.new(0, 8)}, ConfirmFrame)
Custom:Create("UIStroke", {Color = Custom.ColorRGB, Thickness = 1.5, Transparency = 0.5}, ConfirmFrame)

-- Tulisan Pertanyaan
local ConfirmTitle = Custom:Create("TextLabel", {
    Text = "Close ThanHub?",
    Font = Enum.Font.GothamBold,
    TextColor3 = Color3.fromRGB(255, 255, 255),
    TextSize = 14,
...
local ConfirmBtn = Custom:Create("TextButton", {
    Text = "Close",
    Font = Enum.Font.GothamBold,
    TextColor3 = Color3.fromRGB(255, 255, 255),
...
Custom:Create("UICorner", {CornerRadius = UDim.new(0, 6)}, ConfirmBtn)
...
-- Yellow Button (Minimize)
	YellowButton.Activated:Connect(function()
		-- Sembunyikan UI Utama (ThanHub)
		DropShadowHolder.Visible = false
		
		-- Munculkan tombol floating untuk buka lagi
		if Open_Close then 
...
-- Munculkan Overlay pas tombol X diklik
...
-- Kalau user pilih Cancel (Batal)
CancelBtn.Activated:Connect(function()
    local t = TweenService:Create(ConfirmOverlay, TweenInfo.new(0.2), {BackgroundTransparency = 1})
    t:Play()
...
-- Kalau user pilih Close (Konfirmasi Tutup)
ConfirmBtn.Activated:Connect(function()
    ...
    local function AddHover(btn, hoverColor)
        btn.MouseEnter:Connect(function()
            TweenService:Create(btn, TweenInfo.new(0.2), {TextColor3 = hoverColor}):Play()
        end)
    ...
    AddHover(YellowButton, Color3.fromRGB(255, 200, 0)) -- Kuning pas hover
    AddHover(RedButton, Color3.fromRGB(255, 50, 50))    -- Merah pas hover
...
TabButton.Activated:Connect(function()
    -- 1. Cek apakah Tab valid
    if not Tab or CurrentSelectedTab == Tab then return end
    
    -- 2. Efek klik (Cek dulu fungsinya ada atau tidak)
    if CircleClick then
        pcall(function()
            CircleClick(TabButton, game:GetService("Players").LocalPlayer:GetMouse().X, game:GetService("Players").LocalPlayer:GetMouse().Y)
        end)
    end

    -- 3. Cari ChooseFrame (Hanya sekali saja)
    if not GlobalChooseFrame then
        for _, child in pairs(ScrollTab:GetChildren()) do
            local found = child:FindFirstChild("ChooseFrame")
            if found then 
                GlobalChooseFrame = found 
                break 
            end
        end
    end

    -- 4. Reset Tab lama ke visual normal
    if CurrentSelectedTab and CurrentSelectedTab:IsA("GuiObject") then
        local oldStroke = CurrentSelectedTab:FindFirstChildOfClass("UIStroke")
        game:GetService("TweenService"):Create(CurrentSelectedTab, TweenInfo.new(0.2), {BackgroundTransparency = 0.7}):Play()
        if oldStroke then
            game:GetService("TweenService"):Create(oldStroke, TweenInfo.new(0.2), {Transparency = 0.8}):Play()
        end
    end
    ...
    game:GetService("TweenService"):Create(Tab, TweenInfo.new(0.2), {BackgroundTransparency = 0.3}):Play()
    if newStroke then
        game:GetService("TweenService"):Create(newStroke, TweenInfo.new(0.2), {Transparency = 0.2}):Play()
    end

    -- 6. Pindahkan Indikator (ChooseFrame)
    if GlobalChooseFrame then
        local targetY = 12 + (37 * (Tab.LayoutOrder or 0))
        game:GetService("TweenService"):Create(GlobalChooseFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
            Position = UDim2.new(0, 3, 0, targetY)
        }):Play()
    end

    -- 7. Pindah Halaman & Ganti Teks Judul
    if LayersPageLayout and LayersPageLayout:IsA("UIPageLayout") then
        LayersPageLayout:JumpToIndex(Tab.LayoutOrder or 0)
    end
    
    if NameTab and NameTab:IsA("TextLabel") then
        NameTab.Text = _Name or "Tab"
    end
end)
    ...
    return SettingFuncs
      ...
      function Item:AddButton(Config)
        ...
        local function UpdateButtonSize()
          local _Height = 12 + (12 * math.floor(ButtonContent.TextBounds.X / ButtonContent.AbsoluteSize.X))
...
local LastTab = nil -- Buat ngetrack Tab mana yang dibuka sebelum nyari

SearchInput:GetPropertyChangedSignal("Text"):Connect(function()
    local InputText = SearchInput.Text:lower()
    
    if InputText ~= "" then
        -- Simpan Tab terakhir kalau belum ada
        if not SearchResultPage.Visible then
            LastTab = LayersPageLayout.CurrentPage
            SearchResultPage.Visible = true
        end
        
        -- Bersihkan hasil lama
        for _, old in pairs(SearchResultPage:GetChildren()) do
            if not old:IsA("UIListLayout") then old.Parent = old:GetAttribute("OriginalParent") end
        end

        -- Cari di SEMUA Tab
        for _, Page in pairs(LayersFolder:GetChildren()) do
            if Page:IsA("ScrollingFrame") and Page ~= SearchResultPage then
                Page.Visible = false -- Sembunyiin Tab asli
                
                for _, Section in pairs(Page:GetChildren()) do
                    if Section.Name == "Section" then
                        local SectionAdd = Section:FindFirstChild("SectionAdd")
                        if SectionAdd then
                            for _, Feature in pairs(SectionAdd:GetChildren()) do
                                local Title = Feature:FindFirstChildWhichIsA("TextLabel", true)
                                if Title and string.find(Title.Text:lower(), InputText) then
                                    -- Pinjam fiturnya sebentar ke SearchResultPage
                                    if not Feature:GetAttribute("OriginalParent") then
                                        Feature:SetAttribute("OriginalParent", Feature.Parent)
                                    end
                                    Feature.Parent = SearchResultPage
        ...
        -- KALAU KOSONG: Balikin semua fitur ke tempat asalnya
        for _, Feature in pairs(SearchResultPage:GetChildren()) do
            if not Feature:IsA("UIListLayout") then
                Feature.Parent = Feature:GetAttribute("OriginalParent")
            end
        end
        
        SearchResultPage.Visible = false
        if LastTab then
            LastTab.Visible = true
            LayersPageLayout:JumpTo(LastTab)
        end
    end
end)
...
function Item:AddDropdown(Config)
    ...
    DropdownContent:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
        ...
        DropdownContent.Size = UDim2.new(1, -180, 0, 12 + (12 * math.ceil(DropdownContent.TextBounds.X / DropdownContent.AbsoluteSize.X)))
        Dropdown.Size = UDim2.new(1, 0, 0, DropdownContent.AbsoluteSize.Y + 33)
